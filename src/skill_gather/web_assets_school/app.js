...
const stageNames = {
  manifest: "入学登记", media_extract: "教材采集", frame_extract: "画面取样", asr: "课堂转写",
  vision_ocr: "视觉学习", timeline_merge: "证据合并", distill: "课程生成", score: "考试评分", package: "毕业打包"
};
const statusNames = { created: "待入学", running: "修读中", paused: "已休学", completed: "已结课", failed: "挂科", passed: "毕业通过", needs_attention: "需要关注", warning: "警告", needs_review: "待复核" };
...
async function loadResults() {
  ...
  if (!target) return;
  target.innerHTML = '<div class="loading">正在加载成绩单...</div>';
  try {
    ...
    target.innerHTML = visibleResults.length ? visibleResults.map(item => `<div class="run-item"><div class="run-top"><label><input type="checkbox" data-result-id="${escapeHtml(item.run_id)}"> <span class="run-meta">${escapeHtml(item.kind)}</span></label>${statusBadge(item.result_status)}</div><button class="result-read" type="button" data-read-id="${escapeHtml(item.run_id)}">${escapeHtml(item.title || item.topic || item.run_id)}</button><div class="run-meta">成绩 ${Number(item.score) || 0} · 风险 ${(item.risk_flags || []).length}</div><div class="run-meta">${escapeHtml(Object.values(item.artifacts || {}).filter(Boolean).join(" · "))}</div><button class="button secondary result-preview-action" type="button" data-preview-id="${escapeHtml(item.run_id)}">预览产物</button></div>`).join("") : '<div class="loading">暂无匹配成绩。</div>';
...
function renderReadiness(result) {
  ...
  panel.innerHTML = `
    <div class="readiness-head">
      <div><p class="eyebrow">CAMPUS READINESS</p><h2>${escapeHtml(statusLabel)}</h2></div>
...
function updateTopicOperation(topic) {
  ...
  const stageLabels = {
    ...
    processing_sources: "学习已选教材",
    generating: "生成课程和毕业 Skill",
    scoring: "考试与毕业门槛",
  };
  setOperation({
    status: "running",
...
function renderResultDetails(items) {
  ...
  target.innerHTML = items.map(item => {
    const course = item.course ? `<h3>课程档案</h3><pre>${escapeHtml(item.course)}</pre>` : "";
    const skill = item.skill ? `<h3>毕业 Skill</h3><pre>${escapeHtml(item.skill)}</pre>` : "";
...
async function loadTopics(selectFirst = false) {
  ...
  try {
    ...
    if (topicState.topics.length && (selectFirst || !topicState.selectedId)) {
      ...
      topicDetail.innerHTML = '<div class="muted">开设课程后，这里会显示候选教材。</div>';
    }
  } catch (error) {
...
function renderTopics() {
  if (!topicState.topics.length) {
    topicsList.innerHTML = '<div class="loading compact">还没有研究课程。</div>';
    return;
  }
  const labels = { created: "待选课", planning: "课程规划中", awaiting_plan_confirmation: "待确认课题", searching: "选课中", awaiting_selection: "待确认教材", processing_sources: "学习中", generating: "生成课程中", scoring: "考试中", paused: "已休学", completed: "已结课", failed: "挂科" };
  topicsList.innerHTML = topicState.topics.map(topic => `
    <div class="topic-item-row">
      ...
      <button class="icon-button topic-delete-button" type="button" data-topic-delete="${escapeHtml(topic.run_id)}" title="归档课程" aria-label="归档课程">×</button>
    </div>`).join("");
...
async function archiveTopic(runId) {
  ...
  if (!topic) return;
  if (!window.confirm(`确认归档课程“${topic.topic}”吗？归档后会从列表移除，但文件仍保留在 .run-archive/topics 中。`)) return;
  try {
...
async function selectTopic(runId) {
  ...
  renderTopics();
  topicDetail.innerHTML = '<div class="loading compact">正在读取课程档案...</div>';
  try {
...
function renderTopic(topic) {
  ...
  const budgetSummary = `<span class="muted">学期预算：候选 ${Number(budget.max_candidates) || 0} · 教材 ${Number(budget.max_selected_sources) || 0} · 最长 ${Number(budget.max_runtime_sec) || 0} 秒 · 缓存 ${cache.reuse_cache ? "复用" : "关闭"}</span>`;
  const canSearch = topic.status === "created" || topic.status === "awaiting_selection";
  const searchButton = canSearch && topic.execution_mode !== "auto" ? `<button id="topic-search-button" class="button secondary" type="button">${topic.status === "created" ? "搜索教材" : "刷新选课"}</button>` : "";
  const autoButton = topic.execution_mode === "auto" && canSearch ? '<button id="topic-auto-button" class="button primary" type="button">启动自动修读</button>' : "";
  const selectButton = topic.status === "awaiting_selection" && candidates.length ? '<button id="topic-select-button" class="button primary" type="button">确认选课</button>' : "";
  const retryButton = topic.status === "failed" ? '<button id="topic-retry-button" class="button secondary" type="button">申请重修</button>' : "";
  const processButton = topic.status === "processing_sources" && !job.active ? '<button id="topic-process-button" class="button primary" type="button">开始学习已选教材</button>' : "";
  ...
  const confirmation = topic.status === "processing_sources" ? `
    <section class="topic-confirmation" aria-live="polite">
      <div><strong>已确认 ${selectedSources.length} 份教材</strong><span>网页会生成课程总结，公开视频和 GitHub 仓库会写入学习证据。</span></div>
  ...
  const outcome = ["completed", "failed"].includes(topic.status) ? `
    <section class="topic-confirmation topic-outcome" aria-live="polite">
      <div><strong>${topic.status === "completed" ? "课程已经结课" : "本次考试挂科"}</strong><span>${escapeHtml(topic.status === "failed" ? (topic.failure_reason || job.error || "请查看失败审计文件后申请重修。") : "课程与能力产物已生成，请查看下方成绩档案。")}</span></div>
  ...
  topicDetail.innerHTML = `
    ...
    ${candidates.length ? `<div class="candidate-list">${candidates.map(candidate => `
      ...
      </label>`).join("")}</div>` : '<p class="muted">当前没有候选教材。</p>'}`;
  ...
  if (processButtonNode) processButtonNode.addEventListener("click", async () => {
    ...
    try {
      ...
      await loadTopics();
      await selectTopic(topic.run_id);
    } catch (error) { topicMessage.textContent = error.message; topicMessage.className = "form-message error"; processButtonNode.disabled = false; processButtonNode.textContent = "开始学习已选教材"; }
  });
}
...
async function loadRuns(selectFirst = false) {
  if (!state.runs.length) runsList.innerHTML = '<div class="loading">正在读取课程记录...</div>';
  try {
...
function renderRuns() {
  if (!state.runs.length) {
    runsList.innerHTML = '<div class="loading">还没有课程记录。</div>';
    return;
  }
...
async function selectRun(runId) {
  ...
  renderRuns();
  detailPanel.innerHTML = '<div class="loading">正在读取课程档案...</div>';
  try {
...
function renderDetail(run) {
  ...
  const deleteLabel = run.status === "failed" ? "清理挂科产物" : "清理此课程 run";
  ...
  detailPanel.innerHTML = `
    ...
    <div class="detail-grid">
      <div class="metric"><div class="metric-label">最终成绩</div><div class="metric-row"><div class="metric-value">${Number(score.final_score) || 0}</div>${statusBadge(scoreStatus)}</div></div>
...
form.addEventListener("submit", async event => {
  ...
  setOperation({ title: "开设实践课程", detail: "正在提交教材 URL 和考试配置", progress: 8 });
  formMessage.className = "form-message";
    formMessage.textContent = "正在登记实践课...";
  try {
    ...
    formMessage.textContent = "实践课已登记，学习将在后台继续。";
    state.selectedId = result.run_id;
    setOperation({ title: "实践课已登记", detail: `run ${result.run_id} 已进入教务队列`, progress: 18 });
    await loadRuns();
    await selectRun(result.run_id);
  } catch (error) {
    formMessage.className = "form-message error";
    formMessage.textContent = error.message;
    failOperation("登记实践课失败", error.message);
  } finally {
...
topicForm?.addEventListener("submit", async event => {
  event.preventDefault();
  setOperation({ title: "开设研究课程", detail: "正在保存课题和学期预算", progress: 8 });
  topicMessage.className = "form-message";
  topicMessage.textContent = "正在开设课程...";
  try {
    ...
    topicMessage.textContent = "课程已开设，请在右侧完成选课。";
    topicState.selectedId = result.run_id;
    finishOperation("研究课程已开设", result.run_id);
    await loadTopics();
    await selectTopic(result.run_id);
  } catch (error) { topicMessage.className = "form-message error"; topicMessage.textContent = error.message; failOperation("开设研究课程失败", error.message); }
});
...
document.querySelector("#readiness-button").addEventListener("click", async event => {
  const button = event.currentTarget;
  button.disabled = true;
  setOperation({ title: "学校运行状态检查", detail: "正在创建教务检查任务", progress: 4 });
...
async function pollTopics() {
  ...
  try {
    ...
    if (latest && !TOPIC_POLL_STATUSES.has(latest.status)) {
      if (latest.status === "completed") finishOperation("研究课程结课", "课程和毕业 Skill 已生成");
      if (latest.status === "failed") failOperation("研究课程挂科", latest.failure_reason || "请查看课程档案");
    }
  } finally {
